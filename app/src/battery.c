...
static int zmk_battery_update(const struct device *battery) {
    ...
    if (last_state_of_charge != state_of_charge.val1 &&
        ((state_of_charge.val1 < last_state_of_charge) || is_usb_power_present())) {
...
void clear_bat_shutdown(void) { bat_shutdown = false; }
...
void bat_low_check(void) {
    ...
    if ((delta_time > 30 * 1000) || (last_time == 0)) {
        if ((voltage < BAT_VOLTAGE_LOW) && (voltage > BAT_VOLTAGE_SHUTDOWN) &&
            !zmk_usb_is_powered()) {
            led_charge_set_state(LED_BAT_LOW);
            last_time = uptime;
        }
