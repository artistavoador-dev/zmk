...
void set_zmk_keymap(uint8_t layer, uint8_t row, uint8_t column, uint16_t keycode) {
    ...
    case QK_MODS ... QK_MODS_MAX:
        binding.behavior_dev = "KEY_PRESS"; //"&kp";
        binding.param1 = (mod_to_zmk(QK_MODS_GET_MODS(keycode)) << 24) | (HID_USAGE_KEY << 16) |
                         QK_MODS_GET_BASIC_KEYCODE(keycode);
    ...
    // LOG_DBG("layer:%d,row:%d,col:%d,keycode:%d to
    // binding:%s,p1:%x,p2:%x",layer,row,column,keycode,binding.behavior_dev,binding.param1,binding.param2);
                ...
                if (position >= 0) {
                    ...
                    if (memcmp(behavior_dev, "KEY_PRESS", 9) == 0) //&kp
                    {
                        ...
                        case HID_USAGE_KEY:
                            ...
                            if (mod)
                                gen_via_keymaps[layer][row][column] |= mod_to_via(mod) << 8;
                            break;
                        ...
                        gen_via_keymaps[layer][row][column] =
                            QK_LAYER_TAP | (layer << 8) | (keycode);
                        ...
                        if (mod == RSHFT && keycode == RET) {
                            ...
                            gen_via_keymaps[layer][row][column] =
                                QK_MOD_TAP | (mod_kc_to_via(mod) << 8) | (keycode & 0xff);
                    ...
                    dynamic_keymap_set_keycode_no_update(layer, row, column,
                                                         gen_via_keymaps[layer][row][column]);
                    // if(memcmp(behavior_dev,"TRANS",5)&&memcmp(behavior_dev,"KEY_PRESS",9) ||
                    // layer ==3)
                    // if(memcmp(behavior_dev,"TRANS",5) && layer ==3)
                    // LOG_DBG("layer:%d,row:%d,col:%d,pos:%d,binding:%s,p1:%x,p2:%x,via
                    // keycode:%x",layer,row,column,position, binding->behavior_dev,
                    // binding->param1,binding->param2, gen_via_keymaps[layer][row][column]);
                }
            }
        }
    }
